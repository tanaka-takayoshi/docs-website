...
New Relic's [ECS integration](/docs/introduction-amazon-ecs-integration) reports and displays performance data from your [Amazon ECS](https://docs.aws.amazon.com/ecs/index.html) environment. This document explains how to install this integration.

## Install overview

Before you install our ECS integration, we recommend reviewing the [requirements](/docs/introduction-amazon-ecs-integration#requirements).

Here's a brief overview of what happens during the install process:

* For EC2 launch type:

  * The infrastructure agent (`newrelic-infra`) gets deployed onto an ECS cluster as a service using the [daemon scheduling](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/scheduling_tasks.html) strategy.
  * This deployment installs the infrastructure agent in all the container instances of the cluster. The infrastructure agent then monitors ECS and Docker containers.
* For Fargate launch type:

  * The infrastructure agent (`newrelic-infra`) gets deployed as a sidecar in every task to monitor.
   ...
   <CollapserGroup>
     ...
     >
       ...
       1. Download the task definition example with the sidecar container to be deployed:
          ...
          ```
       2. Add the `newrelic-infra` container in this task definition as a sidecar to the task definitions you want monitored.
     </Collapser>
   </CollapserGroup>
...
* Recommended: Install our [ECS cloud integration](https://docs.newrelic.com/docs/integrations/amazon-integrations/aws-integrations-list/aws-ecsecr-monitoring-integration), which gets you other ECS data, including information about clusters and services.
   ...
   <CollapserGroup>
     ...
     >
       ...
       5. Create a service with the [daemon scheduling](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs_services.html) strategy for the registered task:
          ...
          ```
          aws ecs create-service --cluster "YOUR_CLUSTER_NAME" --service-name "newrelic-infra" --task-definition "newrelic-infra" --scheduling-strategy DAEMON
          ```
     </Collapser>
...
* Wait a few minutes and then [look for your data in the UI](/docs/ecs-integration-understand-use-data).
* Recommended: Install our [ECS cloud integration](https://docs.newrelic.com/docs/integrations/amazon-integrations/aws-integrations-list/aws-ecsecr-monitoring-integration), a separate integration which gets you supplementary ECS data, including information about clusters and services.
* See [recommended alert conditions](/docs/ecs-integration-recommended-alert-conditions).
* Understand the [AWS resources](#aws-resources) created by this process.
