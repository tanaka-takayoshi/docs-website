...
If you receive an integration error message from New Relic or error messages in your Prometheus server logs after restarting your Prometheus server, there are several actions you can take to troubleshoot and get data flowing properly. Below are a few tips regarding common issues and error messages. For specific information on how to query for `NrIntegrationError` events, see [Investigate error messages](/docs/integrations/prometheus-integrations/install-configure-remote-write/remote-write-errors-error-messages#investigate-errors) below.
...
1. Run a `NrIntegrationError` query on the error message using NRQL, then look at the **Message** field in the UI to see a description of what went wrong. For example:
