...
![img-integration-k8s@2x.png](./images/img-integration-k8s%402x.png "img-integration-k8s@2x.png") **Kubernetes:** If the scraper is running in Kubernetes, New Relic also adds the following attributes to all the metrics:
...
<CollapserGroup>
  ...
  >
    ![Docker icon](./images/docker-logo-crop.png "Docker icon") **Docker:** This example assumes you are scraping Redis exporters. To view the number of connected Redis clients per endpoint in a cluster:

    ```
    FROM Metric SELECT latest(redis_connected_clients) WHERE clusterName='my-cluster' FACET scrapedEndpoint TIMESERIES
    ```

    ![img-integration-k8s@2x.png](./images/img-integration-k8s%402x.png "img-integration-k8s@2x.png") **Kubernetes:** This example assumes that you have Redis pods with the Redis exporter installed. To view the number of connected Redis clients per pod in the default namespace:
  ...
  <Collapser
    id="xx"
    title="Docker: View average memory free for scraped endpoints"
  >
  ...
  <Collapser
    ...
    title="Kubernetes: View average memory usage for pods in a deployment"
  >
...
To view the NRQL query results for your Prometheus integration's data: Go to **[one.newrelic.com](https://one.newrelic.com) > Query your data**. For more information, see New Relic's [query builder documentation](https://docs.newrelic.com/docs/chart-builder/use-chart-builder/get-started/introduction-chart-builder).
