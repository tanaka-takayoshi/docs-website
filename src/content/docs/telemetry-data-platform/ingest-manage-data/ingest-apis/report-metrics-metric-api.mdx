...
You can find these errors in the `NrIntegrationError` event in the account associated with the Insert API key you used. The `requestId` for each request will be tagged on the `NrIntegrationError` event. See [Troubleshoot a `NRIntegrationError` event](/docs/troubleshoot-nrintegrationerror-event) for more information.
