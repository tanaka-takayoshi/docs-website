...
* `scopeAccounts`: [Scope accounts](/docs/new-relic-one/use-new-relic-one/core-concepts/new-relic-one-workloads-isolate-resolve-incidents-faster#scope-accounts) are the accounts where the entity data is fetched from. Scope accounts must belong to a group under the same master account or enterprise partnership as the workload account.
